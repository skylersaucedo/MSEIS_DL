"""
Use this to make DAS image mask pairs for training/testing DL models.
"""
...
from mseisML_utils import make_image_from_numpy, make_sgy_array, condition_segy
...
def main():
    ...
    ml_data_path_masks = ROOT_DIR +  "\\MLdata\\masks"
    ml_data_path_das = ROOT_DIR +  "\\MLdata\\das"
    ml_data_path_qc = ROOT_DIR +  "\\MLdata\\qc"
        ...
        for i in range(0,len(data_array)):
            ...
            if peek == 'D:\\':
                ...
                try:
                    ...
                    # may need to consider origin time offset
                    # may need to multiply ot by 1000
                    ...
                    ax.imshow(segy_image, cmap="magma", vmin=0,vmax=255,aspect='auto')
                    ax.axes.xaxis.set_visible(False)
                    ax.axes.yaxis.set_visible(False)
                    #plt.show()
                    plt.savefig(ml_data_path_das + '\\' + str(pair_count) + '.png')
                    
                    ### make Masks #####
                    
                    marker_size = 12 # 25 before, looks too large...
                    fig, ax = plt.subplots(figsize=figure_size_in, dpi=dpi)
                    ax.set_facecolor('black')
                    ax.scatter(p_at, channels, label='P arrival', color='blue', alpha=1, s=marker_size)
                    ax.scatter(s_at, channels, label='S arrival', color='red', alpha=1, s=marker_size)
                    ax.set_xlim([0,799])
                    ax.set_ylim([299,0])
                    ax.axes.xaxis.set_visible(False)
                    ax.axes.yaxis.set_visible(False)
                    #plt.show()
                    plt.savefig(ml_data_path_masks + '\\' + str(pair_count) + '.png')
                    
                    pair_count +=1
                    print('pairs made', pair_count)
                    ...
                    ax.set_facecolor('black')
                    ax.imshow(segy_image, cmap="magma", vmin=0,vmax=255,aspect='auto')
                    ax.scatter(p_at, channels, label='P arrival', color='blue', alpha=0.07, s=marker_size)
                    ax.scatter(s_at, channels, label='S arrival', color='red', alpha=0.07, s=marker_size)
                    ax.set_xlim([0,799])
                    ax.set_ylim([299,0])
                    ax.axes.xaxis.set_visible(False)
                    ax.axes.yaxis.set_visible(False)
                    #plt.show()
                    plt.savefig(ml_data_path_qc + '\\' + str(pair_count) + '.png')
